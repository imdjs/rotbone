...
class __RotBones__ : public EditorPlugin
    {
    GODOT_CLASS(__RotBones__,EditorPlugin)
private:
	...
	if(armG)
        ...
        i骨数G = armG->get_bone_count(); PRINT2(, i骨数G, i骨数preG);
		{
            ...
            For(i骨数G,i)
                ...
                VboneG[i]=卍Bone();
                VboneG[i].ξSelf=i;VboneG[i].ξ父=armG->get_bone_parent(i);
                if(VboneG[i].ξ父>-1)
                    {
